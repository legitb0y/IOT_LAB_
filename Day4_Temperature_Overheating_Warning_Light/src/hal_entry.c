...
void hal_entry (void)
{
   ...
   bsp_leds_t leds = g_bsp_leds;
    ...
    while (1)
    {
        ...
        //Change led
        uint32_t alert_pin = leds.p_leds[2];
        if (temperature > 30.0){
            R_BSP_PinWrite((bsp_io_port_pin_t)alert_pin, BSP_IO_LEVEL_HIGH);
        }else{
            R_BSP_PinWrite((bsp_io_port_pin_t)alert_pin, BSP_IO_LEVEL_LOW);
        }
